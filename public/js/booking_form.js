...
async function updateTimeSelect($inputTime, $selectPeople) {
  $inputTime.on('change', async function () {
    ...
    if (selectedTime !== 'undefined' && selectedTime !== null) {
      ...
      for (let auditorium of generatedAuditoriums) {
...
async function updateSeatSelect() {
  ...
  if (bookings !== undefined && bookings.length > 0) {
    //Get occupied seats
    for (let booking of bookings) occupiedSeats.push(...booking.seats);
...
async function clearSelectedSeats() {
  if (generatedAuditoriums.length > 0) {
    for (let auditorium of generatedAuditoriums) {
      await $('#' + auditorium + ' .seat').each(async function () {
        let $this = await $(this);
        if ($this.hasClass('selection-made')) {
          $this.removeClass('selection-made');
          $this.removeClass('selected');
          selectedSeats = [];
          await resetConfirmBooking();
          await $('#cancel_booking_button').show();
        }
      });
    }
  }
...
async function updatePeople($inputAdult, $inputChild, $inputPensioner) {
  ...
  //Clears any seats that are already selected when adding or subtracting people
  await clearSelectedSeats();
  $('#confirm_booking_button').hide();
}
...
async function generateTimeOptions($inputTime) {
  if (availableShows.length > 0 && availableShows !== undefined) {
    ...
    for (let show of availableShows) {
